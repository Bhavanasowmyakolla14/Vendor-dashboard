    ...
    <footer className="bg-sage-950 text-sage-300">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16">
        ...
        <div className="border-t border-white/5 mt-12 pt-8">
          <p className="text-sage-500 text-sm font-bold mb-4">We're in 120+ cities. Popular: </p>
          <div className="flex flex-wrap gap-2">
            {cities.map((city) => (
              <button
                key={city}
                onClick={() => navigate(`/vendors?city=${city}`)}
                className="text-sage-500 text-xs hover:text-sage-300 transition-colors duration-200 after:content-['|'] after:ml-2 after:text-sage-700 last:after:content-[''] font-medium"
              >
                {city}
              </button>
            ))}
        ...
        <div className="border-t border-white/5 mt-8 pt-8 flex flex-col md:flex-row items-center justify-between gap-4">
          ...
          <div className="flex items-center gap-1 text-sage-500 text-sm font-medium">
            <span>Designed & Developed by</span>
            <span className="text-sage-300 font-bold">Panga Yashwant</span>
          </div>
        </div>
      </div>
    </footer>
